...
void loop() 
{
  ...
  delay(1000);  //time for waiting 1000 = 1sec
  lcd.clear(); //clear the LCD
  lcd.print("Ready fot test !!!!");//inscription
  delay(1000);//time for waiting
  lcd.clear();
  ...
  delay(1000);
  lcd.clear();
  if (sensorValue<428) // if sensor value is below 440 ppm
  lcd.print("You are sober."); //inscription
  delay(1000);
  lcd.clear();
  if (sensorValue>=429 && sensorValue<651) // sensor value between 441 and 673 ppm
  lcd.print("You had a drink!"); //inscription
  delay(1000);
  lcd.clear();
  if (sensorValue>652) // sensor value above 674 ppm
  lcd.print("You are drunk!!!"); //inscription
  delay(1000);
  lcd.clear();
  

if (sensorValue < 652)   // compare the sensor value with this one
  { // if sensor value is less than 674ppm 
 ...
 lcd.clear(); 
  ...
  lcd.clear();
  lcd.print("Test not completed"); // inscription for test failed
  delay(1000);
  lcd.clear();
}
